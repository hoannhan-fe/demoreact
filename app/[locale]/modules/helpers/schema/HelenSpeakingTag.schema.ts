...
export type HelenSpeakingTagSchema = InferType<typeof formSchema>;

export function useFormHelenSpeakingTag() {
  const form = useForm<HelenSpeakingTagSchema>({
  ...
  return {
    form,
    formSchema,
    isError: isError<HelenSpeakingTagSchema>(form),
    getErrorMessage: getErrorMessage<HelenSpeakingTagSchema>(form),
  };
}
